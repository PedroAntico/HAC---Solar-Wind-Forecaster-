...
- Métricas de fase temporal (timing do mínimo, área integrada, recovery slope).
- Benchmark contra persistência.
"""
...
# =========================
# ALINHAMENTO COM LAG FIXO
# =========================
def align_omni_to_dst_fixed(omni_df, dst_df, lag_minutes=LAG_MINUTES):
    """Alinha OMNI ao Dst usando lag fixo (preserva extremos)."""
...
# =========================
def persistence_forecast(dst_obs, steps=1):
...
def validate_event(config_core, df_aligned, name, start, end, physics_config):
    ...
    phase = phase_metrics(event['time_tag'].values, event['dst'].values, dst_pred)

    # Benchmark: persistência
    dst_persist = persistence_forecast(event['dst'].values, steps=1)
    metrics_persist = evaluate_event(
        time=event['time_tag'].values,
        dst_obs=event['dst'].values,
        dst_pred=dst_persist
    )
    phase_persist = phase_metrics(event['time_tag'].values, event['dst'].values, dst_persist)
    ...
    print(f"\n   📊 Persistência:")
    print(f"      Correlação: {metrics_persist['correlation']:.3f}")
    print(f"      MAE: {metrics_persist['MAE']:.1f} nT")
    print(f"      Timing erro: {phase_persist['min_timing_error_min']:.1f} min")

    # Plot
    plt.figure(figsize=(12, 5))
    plt.plot(event['time_tag'], event['dst'], 'b-', label='Dst Kyoto', linewidth=2)
    plt.plot(event['time_tag'], dst_pred, 'r--', label='HAC++', linewidth=2)
    plt.plot(event['time_tag'], dst_persist, 'g:', label='Persistência', linewidth=1.5)
...
def main():
    ...
    # Calibração global
    print("\n⚙️ Preparando conjunto de treino...")
    ...
    # Configuração física padrão (sem ML residual para validação pura)
