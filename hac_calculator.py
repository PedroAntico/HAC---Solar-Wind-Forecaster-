"""
HAC - Heliospheric Accumulated Coupling (Versão Final com Baselines e Validação Robusta)
...
# Termo fonte (modelo principal)
USE_DENSITY = True
USE_BY_CLOCK = True
USE_DYNAMIC_PRESSURE = True
P0 = 2.0                     # nPa
ALPHA_PDYN = 1/6             # expoente da pressão dinâmica

# Valores típicos para normalização
V_TYPICAL = 400.0
BZ_TYPICAL = 5.0
B_TYPICAL = 7.0
PDYN_TYPICAL = 2.0
...
def compute_normalization_constant():
    S_typical = (V_TYPICAL * BZ_TYPICAL * np.sqrt(B_TYPICAL) *
                 (PDYN_TYPICAL / P0) ** ALPHA_PDYN)
    return S_typical / HAC_QUIET_TARGET

SOURCE_NORM_CONSTANT = compute_normalization_constant()

# Adaptação da memória à fonte
BETA_SOURCE = 0.5
...
# Clipping físico
MAX_SOURCE_PHYSICAL = 10000.0

# Suavização da derivada (apenas para gráfico)
...
# Preditor (fixo em 'hac' para os modelos, mas a validação pode usar HAC ou derivada)
PREDICTOR_TYPE = 'hac'
...
def compute_source_advanced(Bx, By, Bz, V, N):
    ...
    if USE_DENSITY and USE_DYNAMIC_PRESSURE:
        ...
        base *= dyn_factor
    base = np.clip(base, 0, MAX_SOURCE_PHYSICAL)
...
def compute_source_simple(df):
    ...
    south = Bz < 0
    source[south] = -Bz[south] * V[south] / 1000.0   # /1000 mantém na mesma ordem
    return source
...
def compute_source_akasofu(df):
    ...
    # Normalização empírica para escala comparável
    return epsilon / 1e6

def integrate_hac(df, source_func, calib_factor=1.0,
                  beta_source=BETA_SOURCE, loss_exp=LOSS_EXPONENT, h0=H0):
    """
    Função genérica de integração para qualquer fonte.
    Retorna DataFrame com HAC e derivada.
    """
    times = df['time_tag'].values
    Vsw = df['speed'].values
    source = source_func(df)
    ...
    tau_hours = TAU_BASE_HOURS * (V_REF / np.maximum(Vsw, 1.0)) ** TAU_POWER
    tau_hours = np.clip(tau_hours, TAU_MIN_HOURS, TAU_MAX_HOURS)
    ...
    for i in range(1, len(hac)):
        ...
        source_eff = source[i]   # sem saturação

        # Perda não linear (se loss_exp != 1)
        if loss_exp != 1.0:
...
# ============================
# FUNÇÕES DE VALIDAÇÃO (mesmas de antes, mas com suporte a threshold_hac)
# ============================
...
def compute_metrics(y_true, y_pred, threshold_dst=None, threshold_hac=None, hac_vals=None):
    ...
    if threshold_dst is not None and threshold_hac is not None and hac_vals is not None:
        true_storm = y_true > threshold_dst   # y_true é -dst_smooth (positivo)
...
def calibrate_hac_nonlinear(df_hac_raw, dst_series, delay=0, min_hac=0.01,
                            smooth_dst_hours=DST_SMOOTH_HOURS, predictor='hac',
    ...
    if predictor == 'hac':
        ...
        X_col = 'HAC_raw'
    mask = (merged['dst_smooth'] < 0) & (merged['dst_smooth'].abs() > MIN_DST_CALIB) & (merged[X_col] > min_hac)
    if mask.sum() < 5:
...
def get_hac_threshold_from_calibration(calib_slope, calib_intercept, target_dst=50, transform='log'):
    """Inverte a calibração para encontrar HAC que corresponde a um dado |Dst|."""
    if calib_slope is None or calib_intercept is None:
        return None
    if transform == 'log':
        # target_dst = exp(slope * log(1+HAC) + intercept) - 1
        # log(1+HAC) = (log(target_dst+1) - intercept) / slope
        log1p_hac = (np.log1p(target_dst) - calib_intercept) / calib_slope
        hac_thresh = np.expm1(log1p_hac)
    elif transform == 'sqrt':
        # target_dst = (slope * sqrt(HAC) + intercept)^2
...
def test_physics():
    ...
    S = compute_source_advanced(np.array([Bx]), np.array([By]), np.array([Bz]),
                                np.array([V]), np.array([N]))[0]
    ...
    V_storm = 800.0
    Bz_storm = -20.0
    S_storm = compute_source_advanced(np.array([Bx]), np.array([By]), np.array([Bz_storm]),
                                      np.array([V_storm]), np.array([N_storm]))[0]
    print(f"3. Fonte(storm) = {S_storm:.2f}")
    assert S_storm > 10 * S, f"Fonte de tempestade não amplificou: {S_storm} vs {S}"
    # 4. Normalização
    print(f"4. SOURCE_NORM = {SOURCE_NORM_CONSTANT:.2f}")
    assert 30 < SOURCE_NORM_CONSTANT < 100, f"Normalização fora da faixa: {SOURCE_NORM_CONSTANT}"
...
def main():
    ...
    parser.add_argument('--alpha_pdyn', type=float, default=ALPHA_PDYN,
                        help='Expoente da pressão dinâmica (teste: 0.0, 0.5, 1.0)')
    args = parser.parse_args()

    # Ajustar parâmetros conforme simplificação
    global ALPHA_PDYN, SOURCE_NORM_CONSTANT, BETA_SOURCE, LOSS_EXPONENT, USE_DYNAMIC_PRESSURE
    if args.simplify:
        print("\n🔧 Modo SIMPLIFICADO ativado: removendo perda não linear, Pdyn e adaptação.")
        BETA_SOURCE = 0.0
        LOSS_EXPONENT = 1.0
        USE_DYNAMIC_PRESSURE = False
        # Recalcular normalização sem Pdyn
        ALPHA_PDYN = args.alpha_pdyn
        SOURCE_NORM_CONSTANT = compute_normalization_constant()
    else:
        ALPHA_PDYN = args.alpha_pdyn
        SOURCE_NORM_CONSTANT = compute_normalization_constant()
        print(f"\n📊 Usando α_pdyn = {ALPHA_PDYN:.2f}")
        print(f"   Normalização física: SOURCE_NORM = {SOURCE_NORM_CONSTANT:.2f}")

    print("\n" + "="*70)
    print(f"🛰️  HAC - Modelo: {args.model.upper()}")
    print("="*70)
    ...
    dst = dst.sort_values('time_tag')
    # Criar grade completa de tempo do OMNI
    ...
    if args.model == 'simple':
        source_func = lambda d: compute_source_simple(d)
    elif args.model == 'akasofu':
        source_func = lambda d: compute_source_akasofu(d)
    else:  # 'hac'
        source_func = lambda d: compute_source_advanced(
            d['bx_gsm'].values, d['by_gsm'].values, d['bz_gsm'].values,
            d['speed'].values, d['density'].values
        )

    # Calcular HAC (primeira passagem sem calibração)
    df = integrate_hac(df, source_func, calib_factor=1.0,
                       beta_source=BETA_SOURCE, loss_exp=LOSS_EXPONENT, h0=H0)
    ...
    if args.model == 'hac':
        ...
        df_simple = integrate_hac(df, lambda d: compute_source_simple(d), calib_factor=1.0,
                                  beta_source=BETA_SOURCE, loss_exp=LOSS_EXPONENT, h0=H0)
        df_akasofu = integrate_hac(df, lambda d: compute_source_akasofu(d), calib_factor=1.0,
                                   beta_source=BETA_SOURCE, loss_exp=LOSS_EXPONENT, h0=H0)
    ...
    if not args.no_calibrate:
        ...
        calib_slope, calib_intercept = calibrate_hac_nonlinear(
            df_hac_raw, dst_df, delay=best_delay, min_hac=0.01,
        ...
        if calib_slope is None:
            ...
            if not (0.01 < linear_factor < 100):
                linear_factor = 1.0
            df = integrate_hac(df, source_func, calib_factor=linear_factor,
                               beta_source=BETA_SOURCE, loss_exp=LOSS_EXPONENT, h0=H0)
            if args.model == 'hac':
                df_simple = integrate_hac(df, lambda d: compute_source_simple(d), calib_factor=linear_factor,
                                          beta_source=BETA_SOURCE, loss_exp=LOSS_EXPONENT, h0=H0)
                df_akasofu = integrate_hac(df, lambda d: compute_source_akasofu(d), calib_factor=linear_factor,
                                           beta_source=BETA_SOURCE, loss_exp=LOSS_EXPONENT, h0=H0)
                baseline_simple_vals = df_simple['HAC'].values
                baseline_akasofu_vals = df_akasofu['HAC'].values
            df_hac = df[['time_tag', 'HAC', 'dHAC_dt']].copy()
    ...
    if len(merged) > 10:
        y_true = -merged['dst_smooth'].values
        y_pred = predict_from_calibration(merged['HAC_raw'].values, calib_slope, calib_intercept, CALIBRATION_TRANSFORM)
...
if __name__ == "__main__":
    # Executar testes de unidade antes de rodar
    test_physics()
    main()
