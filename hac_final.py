    ...
    def compute_hac_system(self, df, calibration_mode=False):
        ...
        for i in range(1, n):
            ...
            if regime_adv.startswith('CME'):
                ...
                simple_regime = 'Quiet'
            last_regime = simple_regime

            if simple_regime == 'CME':
                mem_gain = self.config.ENERGY_MEMORY_GAIN_CME
            elif simple_regime == 'HSS':
            ...
            prev_tail_energy = tail_energy[i-1]
            ...
            plasma_tau_extension = self.config.PLASMA_TO_RING * plasmasphere_energy[i] / self.config.TAU_PLASMASPHERE * 6.0
        ...
        # Previsão (forecast) usando último regime
        if last_regime == 'CME':
        ...
        for h in [1, 2, 3]:
            ...
            plasma_fut = plasmasphere_energy[-1]
        ...
        self.results.update({
            ...
            'plasmasphere_energy': plasmasphere_energy,
            'Pdyn': pdyn,
...
def main():
    ...
    df_out = pd.DataFrame({
        ...
        'plasmasphere_energy': model.results['plasmasphere_energy'],
